...
class Venus:
    def __init__(
        ...
        self.venus = venusplc.VENUSController(
            "/home/damon/venus_data_project/config/config.ini"
        )
    ...
    def change_superconductors(self, Igoal):
        ...
        Inow = np.array(
            [venus.read(["inj_i"]), venus.read(["ext_i"]), venus.read(["mid_i"])]
        )  # current currents
        ...
        diffup = np.array([0.03, 0.04, 0.08])
        diffdown = np.array([0.06, 0.10, 0.25])
        Ioff = Igoal * 2.0
        for i in range(len(Ioff)):
            if direction[i] > 0:
                Ioff[i] = Ioff[i] - direction[i] * diffup[i]
            if direction[i] < 0:
                Ioff[i] = Ioff[i] - direction[i] * diffdown[i]
        ...
        while (
            ...
            if time.time() - time_start_change > 300.0:
                ...
                for i in range(3):
                    print(
                        "in here. i=%i, Inow[i]=%f, Igoal[i]=%f, done[i]=%i"
                        % (i, Inow[i], Igoal[i], done[i])
                    )
                    if (
                        np.abs(Inow[i] - Igoal[i]) < 0.08 and done[i] == 0
                    ):  # for small change problem
                        Igoal[i] = Igoal[i] - 0.01 * np.sign(Inow[i] - Igoal[i])
                        venus.write({names[i]: Igoal[i]})
                        print("stuck with small difference.  resetting Igoal")
                        time_start_change = time.time()
                    if (
                        np.abs(Inow[i] - Igoal[i]) >= 0.08
                    ):  # for some reason done going to 1 but not changing I goal
                        done[i] = 0
                        venus.write({names[i]: Igoal[i]})
                        print("stuck with big difference. re-requesting Igoal")
                        time_start_change = time.time()
            checkdone[:, :-1] = checkdone[:, 1:]
            checkdone[:, -1] = np.abs(Inow - Igoal)
        # print('done setting field ')

    def set_mag_currents(self, inj, ext, mid):
        """Set the magnetic currents on the coils."""
        for v, lim in zip(
            [inj, ext, mid], [self.inj_limits, self.ext_limits, self.mid_limits]
        ):
    ...
    def get_beam_current(self):
        ...
        venus = self.venus
        Ifc = (
            venus.read(["fcv1_i"]) * 1e6
        )  # faraday cup current (single species current) in microamps
        return Ifc

    def monitor(self, t_start, t_program_start, output_file):
        venus = self.venus
        Ifc = (
            venus.read(["fcv1_i"]) * 1e6
        )  # faraday cup current (single species current) in microamps
        Idrain = venus.read(
            ["extraction_i"]
        )  # drain current ~ total extracted beam current [mA]
...
def black_box_function(A, B, C):
    ...
    while time.time() < t_end:
        venus.monitor(t_start, t_program_start, writefile)
